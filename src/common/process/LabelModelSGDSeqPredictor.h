...
public:
  ...
  virtual void predict() {
    ...
    for (int j = 0; j < testSamples.size(); ++j) {
      ...
      vector<Prediction> results = labelModel.ExtractResults(hg, adists);
      writeBackSampleByPredict(testSamples[j], results);
  ...
  virtual void writeBackSampleByPredict(SimpleClass& SampleToWrite, vector<Prediction>& prediction) {};
